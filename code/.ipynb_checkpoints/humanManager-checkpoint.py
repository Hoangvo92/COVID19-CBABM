...
class HumanAgent(GeoAgent):
    ...
    def __init__(
        ...
        if self.random.random() < init_infected*4 and self.profession != "Healthcare":
            self.atype = "infected"
    ...
    def dead(self):
        ...
        if self.atype != "recovered":
            if self.age >= 55 or self.age <= 12:
                if self.model.steps % 12 == 0:
                    if self.random.random() < self.death_risk:
                        if self.resistance_level < 0.1 and self.infection_level == 1:
                            self.atype = "dead"  
...
def getPreferences(string):
    people = []
    with open('DatabaseAdaptor/preferences.csv') as csv_file:
