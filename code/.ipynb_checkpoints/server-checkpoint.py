...
model_params = {
   ...
   # "poi_size": UserSettableParameter('slider', 'Number of Points of Interest', 1000, 100, 100000, 90),
    "pop_size": UserSettableParameter('number', 'Population size', value=10000),
    "poi_size": UserSettableParameter('number', 'Number of Points of Interest', value=4000),
    "n_days": UserSettableParameter('number', 'Days of simulation', value=30),
    "weather": UserSettableParameter('number', 'Weather effect(from 1 to 4)', value=1), #no effect=1, normal=2, strong=3, very strong=4
    "policy": UserSettableParameter('number', 'Intervence Policy (from 1 to 4)', value=2),#no policy =1, good policy=2, very good policy= 3, strict and lockdown = 4
    "init_infected": UserSettableParameter(
        "slider", "Fraction initial infection", 0.05, 0.00, 1.0, 0.005   #0.01
    ),   #https://covidactnow.org/us/new_york-ny/?s=31661989
    "init_vaccinated": UserSettableParameter(
        "slider", "Fraction initial vaccination", 0.575, 0.00, 1.0, 0.005   
    ),
    "exposure_distance": UserSettableParameter(
        "slider", "Exposure distance", 1000, 100, 10000, 100
    ),
}
